...
class Matrix { 
    ...
    constructor(htmlObject) { // passes the DOM text container into our Javasctipt file 
        ...
        this.duration = 40; 
        this.animatePhrase= this.animatePhrase.bind(this); // function needs to be bound because we are using it as a callback arguement in requestAnimationFrame
    }

    getTextData(newPhrase) { // this function gets all the vital information from both phrases
        const promise = new Promise(resolve => this.myPromistAtt = resolve); // makes a promise to be TRUE if animation if completed.
    ...
    animatePhrase() {
        ...
        if (complete === this.allData.length) { // checks to see if our complete flag equals our greatest phrase length 
            this.myPromistAtt() ; // if it does, it means all characters have been converted 
        }else {

            window.requestAnimationFrame(theMatrix.animatePhrase); // updates the animation befoe the callback update function is called - so creating a loop
            this.frame++; // frame value will be increase by 1. This will be done first before this.animatePhrase is called because Javascript is a single threaded language
        }            
